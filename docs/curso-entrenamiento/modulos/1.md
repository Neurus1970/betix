# Módulo 1 — El entorno local estandarizado

← [Volver al temario](../temario.md) | ← [Módulo 0](0.md)
...
- Tener Betix corriendo en tu máquina en menos de 10 minutos
- Entender qué servicio hace qué y por qué hay tres
...
Agregá estas variables a tu perfil de shell (`~/.bashrc`, `~/.zshrc` o equivalente):

```bash
export JIRA_USERNAME="tu-email@dominio.com"
export JIRA_API_TOKEN="tu-api-token"
```

Recargá el perfil (`source ~/.zshrc`) y reiniciá VS Code para que Claude Code tome los valores.
...
Betix no es una app monolítica. Está compuesta por **tres procesos independientes** que se comunican entre sí. Entender esta separación es clave antes de correr cualquier comando.

```mermaid
graph LR
    U(["👤 Usuario<br/>(Browser)"])

    subgraph Betix
        FE["🌐 Frontend<br/>nginx :8080<br/><small>Sirve HTML/CSS/JS + D3.js</small>"]
        API["⚙️ Betix API<br/>Node.js :3000<br/><small>Proxy HTTP + caché Redis</small>"]
        CORE["🧠 Betix Core<br/>Python Flask :5000<br/><small>Toda la lógica de negocio</small>"]
        REDIS[("🔴 Redis :6379<br/><small>Caché de respuestas</small>")]
        DB[("🐘 PostgreSQL :5432<br/><small>Datos de tickets</small>")]
    end
...
> **Regla crítica:** La lógica de negocio **solo vive en `core/`**. Si necesitás hacer un cálculo o consultar la base de datos, lo hacés en Python. Node.js solo reenvía requests y cachea respuestas. Ver [CLAUDE.md — Critical](../../../CLAUDE.md#critical).
...
sequenceDiagram
    ...
    Note over DB: healthcheck:<br/>pg_isready -U betix
    ...
    Note over Redis: healthcheck:<br/>redis-cli ping

    Redis-->>DC: ✅ Healthy

    DC->>Core: Iniciar Flask (depende de DB)
    Note over Core: healthcheck:<br/>GET /healthz
...
```mermaid
graph TD
    L1["📦 Nivel 1 — Contexto<br/><small>¿Qué es Betix y quién lo usa?</small><br/><small>Audiencia: negocio</small>"]
    L2["📦 Nivel 2 — Contenedores<br/><small>¿Qué servicios lo componen?</small><br/><small>Audiencia: arquitectos</small>"]
    L3["📦 Nivel 3 — Componentes<br/><small>¿Qué hay dentro de cada servicio?</small><br/><small>Audiencia: desarrolladores</small>"]
    L4["📦 Nivel 4 — Código<br/><small>Clases, funciones, detalles</small><br/><small>Audiencia: IDE / revisión</small>"]
...
**Siguiente módulo →** [Módulo 2 — Git como contrato de equipo](2.md)
