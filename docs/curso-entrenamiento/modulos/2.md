...
> `main` solo recibe merges de `develop`. **Nunca** se pushea directamente a `main`. Si necesitás un fix urgente en producción, usás una rama `hotfix/` que sale de `main` y vuelve a `main` (y se cherry-pickea a `develop`).
