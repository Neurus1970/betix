...
> **User Story BETIX-99:** *"Como analista de negocio, quiero ver el total de ingresos de un mes a elección en el dashboard de Betix, para tener una visión rápida del rendimiento mensual sin navegar a múltiples reportes."*
...
flowchart LR
    ...
    FLASK --> PROXY["Proxy\nNode.js src/"]
    PROXY --> FRONT["Widget\nDashboard"]
    FRONT --> TEST["make test\n✅ todos pasan"]
    TEST --> PR["PR a develop\nconventional commits"]
    PR --> CI["CI corre\nsha-abc1234 → ECR"]
...
**Tipo:** Story
**Clave:** BETIX-99 (o el siguiente número disponible)
...
git pull
git checkout -b feature/BETIX-99-ventas-mes-dashboard
```
...
Toda la lógica de negocio vive en `core/`. El [Principio 1](../../principios-fundamentales.md) lo expresa sin ambigüedad: *"Business logic lives in `core/` only. Never duplicate in Node.js."*
...
```bash
git log --oneline feature/BETIX-99-ventas-mes-dashboard
```
...
```bash
git push -u origin feature/BETIX-99-ventas-mes-dashboard
gh pr create --title "feat(BETIX-99): monthly income widget in dashboard" \
  --body "Closes BETIX-99. Adds GET /ventas/mensual endpoint (core + proxy + frontend widget)." \
  --base develop
```
...
Repasá los 5 principios fundacionales (`docs/principios-fundamentales.md`) y compará con lo que acabás de hacer:
...
| `.claude/skills/add-endpoint.md` | Playbook reutilizable para agregar un endpoint completo |
