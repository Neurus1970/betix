# Claude Code como herramienta de SDLC
...
graph LR
    subgraph "Sin Claude Code"
        DEV1(["👤 Developer"]) -->|"lee código<br/>10 archivos"| ENTIENDE["Entiende<br/>el contexto"]
        ENTIENDE -->|"escribe<br/>a mano"| IMPL["Implementa"]
        IMPL -->|"busca<br/>cómo testear"| TEST["Escribe tests"]
    end

    subgraph "Con Claude Code"
        DEV2(["👤 Developer"]) -->|"describe<br/>lo que necesita"| CLAUDE["🤖 Claude Code<br/>(lee el repo)"]
        CLAUDE -->|"propone<br/>cambios"| IMPL2["Implementa"]
        CLAUDE -->|"agente<br/>testing"| TEST2["Escribe tests"]
    end
...
La plataforma establece la convención de definir **agentes especializados** en `.claude/agents/` — perfiles de Claude con contexto específico sobre un área del sistema. Cada proyecto del equipo puede definir sus propios agentes según su arquitectura.

Betix implementa cuatro agentes, cada uno con conocimiento profundo de su área:

```mermaid
graph TD
    CLAUDE(["🤖 Claude Code<br/>(conversación principal)"])

    CLAUDE -->|"toca core/ o src/<br/>solo lógica de prod"| MS["🐍 microservices<br/>Python Flask + Node.js proxy"]
    CLAUDE -->|"cualquier tarea de tests"| TEST["🧪 testing<br/>pytest + Jest + Cucumber"]
    CLAUDE -->|"docker-compose, k8s,<br/>terraform, CI/CD"| INFRA["⚙️ infra<br/>Docker + K8s + Terraform + GitHub Actions"]
    CLAUDE -->|"frontend/ o src/public/<br/>HTML/CSS/JS/D3.js"| FE["🎨 frontend<br/>nginx + D3.js"]
...
La plataforma establece que **todo proyecto del equipo debe mantener un `CLAUDE.md`** en la raíz del repositorio. Este archivo es el "contrato" entre el equipo y Claude: describe la arquitectura, las reglas, los comandos y las convenciones del proyecto específico.

En Betix, `CLAUDE.md` define:
...
| [`docs/claude-en-betix.md`](../../claude-en-betix.md) | Guía completa: sub-agentes, sus responsabilidades, cómo colaboran, hooks y skills |
