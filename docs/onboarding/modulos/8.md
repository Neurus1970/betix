...
```mermaid
graph TD
    COMMIT["Nuevo commit"] --> Q1{"¿Rompe la API\nde los clientes actuales?"}
    Q1 -->|Sí| MAJOR["⬆ MAJOR\n1.3.0 → 2.0.0\nBreaking change"]
    Q1 -->|No| Q2{"¿Agrega\nfuncionalidad nueva?"}
    Q2 -->|Sí| MINOR["⬆ MINOR\n1.3.0 → 1.4.0\nNueva feature"]
    Q2 -->|No| PATCH["⬆ PATCH\n1.3.0 → 1.3.1\nBug fix o corrección"]
...
flowchart LR
    subgraph "Commits en main"
        C1["fix: corrección typo\n→ no cuenta"]
        C2["feat(BETIX-43): módulo versionado\n→ MINOR"]
        C3["fix(BETIX-44): error en stats\n→ PATCH"]
    end
...
graph TD
    subgraph "Rama feature / develop"
        F1["git push\nfeature/BETIX-43-..."] --> SHA["Tag: sha-abc1234\n(inmutable, rastreable)"]
    end

    subgraph "Rama main — release candidate"
        RC1["release PR mergeado\ncon sufijo -rc"] --> RCTAG["Tag: 1.4.0-rc.1\n(candidato a release)"]
    end

    subgraph "Rama main — release estable"
        S1["release PR mergeado\nen main"] --> STABLE["Tag: 1.4.0\n(estable)"]
        S1 --> LATEST["Tag: latest\n(siempre el último estable)"]
    end
...
graph LR
    ...
    subgraph STAGING["🔶 Staging"]
        S1["1.4.0-rc.1\nValidación pre-producción\nSe promueve desde sha"]
    end

    subgraph PROD["🚀 Producción"]
        P1["1.4.0\nVersión estable y fija\nNunca usar 'latest' en prod"]
    end
