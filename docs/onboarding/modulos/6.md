...
- Explicar qué es FinOps, por qué la plataforma exige tagging consistente y cómo `finops/tagging-taxonomy.yaml` actúa como fuente única de verdad para tags y presupuestos
...
## 5. FinOps: visibilidad y control de costos

### Por qué la plataforma lo exige

Sin etiquetas consistentes en los recursos de AWS, es imposible saber cuánto cuesta cada producto, equipo o entorno. Una cuenta con decenas de servicios corriendo genera una factura única — sin forma de saber si el 80% del costo viene de EKS, de RDS, o de un NAT Gateway que nadie recuerda haber creado.

La plataforma exige que todo recurso de infraestructura tenga un conjunto mínimo de tags que permitan responder: **¿quién lo creó, para qué producto, en qué entorno, y quién paga?** Eso es [FinOps](../glosario.md#finops): la práctica de gestionar el gasto cloud con la misma disciplina que el código.

### La taxonomía de tagging: fuente única de verdad

Betix centraliza toda la configuración de FinOps en un solo archivo: [`finops/tagging-taxonomy.yaml`](../../../finops/tagging-taxonomy.yaml). Este archivo es leído tanto por Terraform como por los workflows de CI — es la fuente única de verdad para tags y presupuestos.

```yaml
# finops/tagging-taxonomy.yaml (extracto)
required_tags:
  - key: product       # ej: betix
  - key: environment   # dev | uat | prod (se setea en apply, no hardcodeado aquí)
  - key: owner         # equipo responsable: platform-team
  - key: cost-center   # código contable: CC-001
  - key: created-by    # terraform | github-actions
```

Los cinco tags son **obligatorios** en todos los recursos. El archivo también define los presupuestos por entorno:
...
> **Por qué el presupuesto anual es el número primario:** el anual define el compromiso de gasto del año. Los valores mensual y semanal se derivan matemáticamente (`anual / 12`, `anual / 52`) para que nunca haya inconsistencias entre horizontes temporales.

### Cómo los tags llegan a todos los recursos sin repetirlos

En `terraform/main.tf`, los tags se aplican una sola vez al provider de AWS mediante `default_tags`:
...
locals {
  ...
  finops_tags = {
    ...
    environment   = var.environment          # viene de -var environment=dev
...
`default_tags` es el mecanismo del provider de Terraform para AWS: cualquier recurso que se crea hereda automáticamente esos tags. No hace falta repetir los tags en cada `resource` block — si alguien agrega un nuevo recurso sin tags, igual los recibe del provider.

```mermaid
flowchart LR
    YAML["finops/tagging-taxonomy.yaml\n(fuente única de verdad)"]
    MAIN["terraform/main.tf\nyamldecode → locals → default_tags"]
    VPC["aws_vpc.betix\n+ 5 tags FinOps"]
    EKS["aws_eks_cluster.betix\n+ 5 tags FinOps"]
    ECR["aws_ecr_repository.betix\n+ 5 tags FinOps"]
    RDS["aws_db_instance.betix\n+ 5 tags FinOps"]

    YAML -->|yamldecode| MAIN
    MAIN -->|herencia automática| VPC
    MAIN -->|herencia automática| EKS
    MAIN -->|herencia automática| ECR
    MAIN -->|herencia automática| RDS
```

### Presupuestos AWS y alertas

`terraform/budgets.tf` crea tres presupuestos en AWS Budgets, filtrados por el tag `product=betix`:

| Budget | Período | Alertas |
|--------|---------|---------|
| `betix-dev-monthly` | Mensual | 70 % / 80 % / 90 % del límite |
| `betix-dev-annual` | Anual | 70 % / 80 % / 90 % del límite |
| `betix-dev-weekly` | Semanal | 70 % / 80 % / 90 % del límite |

Cada alerta dispara un mensaje al [SNS topic](../glosario.md#sns-simple-notification-service) `betix-finops-alerts-dev` y un email directo a `finops@tecnoaccion.com`. El filtro `product=betix` hace que AWS solo cuente los recursos con ese tag — si un recurso no está taggeado correctamente, su costo no aparece en el presupuesto.
...
sequenceDiagram
    ...
    participant EMAIL as finops@tecnoaccion.com

    AWS->>BUDGET: gasto acumulado supera 70% del límite mensual
    BUDGET->>SNS: publica alerta
    SNS->>EMAIL: envía notificación
    Note over EMAIL: "Alerta: betix-dev llegó al 70% del budget mensual ($140 de $200)"
```

### Validación en CI: check-tags.py

Para garantizar que ningún recurso nuevo se cree sin los tags obligatorios, el script `scripts/check-tags.py` valida el plan de Terraform antes de aplicarlo:
...
El script lee el plan JSON, extrae los `tags_all` de cada recurso que se va a crear o modificar, y verifica que tenga los cinco tags. Si falta alguno, el CI falla con detalle de qué recurso y qué tags le faltan:

```
[FALLO] 1 recurso(s) sin tags obligatorios:

  Recurso: aws_security_group.custom
  Tipo:    aws_security_group
  Faltan:  cost-center, owner

Agregar los tags faltantes en finops/tagging-taxonomy.yaml o en el resource block.
```

Dado que `default_tags` del provider cubre todos los recursos, en la práctica esta validación actúa como red de seguridad ante errores de configuración o recursos creados fuera del módulo principal.

### El ciclo FinOps en Betix

```mermaid
flowchart TD
    YAML["finops/tagging-taxonomy.yaml\neditar para cambiar tags o presupuesto"]
    PLAN["terraform plan\ngenera tfplan.json"]
    CHECK["scripts/check-tags.py\nvalida tags obligatorios"]
    APPLY["terraform apply\napplica tags a todos los recursos"]
    BUDGET["AWS Budgets\nfiltra costos por product=betix"]
    ALERT["Alerta 70/80/90%\nSNS + email"]

    YAML --> PLAN --> CHECK
    CHECK -->|OK| APPLY --> BUDGET --> ALERT
    CHECK -->|FALLO| PLAN
```

> **Para cambiar el presupuesto:** editá `finops/tagging-taxonomy.yaml`, abrí un PR, y el reviewer puede ver exactamente cuánto cambia el límite antes de aprobarlo. No hay número hardcodeado en Terraform.

---

## 6. El loop diagrama ↔ infraestructura
...
## 7. Ejercicio — Interpretar y actualizar con el agente infra
...
mindmap
  root((Módulo 6))
    ...
    FinOps
      tagging-taxonomy.yaml: fuente única de verdad
      5 tags obligatorios: product, environment, owner, cost-center, created-by
      default_tags: herencia automática en todos los recursos
      AWS Budgets: mensual, anual, semanal por entorno
      Alertas: 70/80/90% del límite por SNS + email
      check-tags.py: validación de tags en CI
...
| [`finops/tagging-taxonomy.yaml`](../../../finops/tagging-taxonomy.yaml) | Fuente única de verdad: 5 tags obligatorios FinOps + presupuestos por entorno |
| [`terraform/budgets.tf`](../../../terraform/budgets.tf) | Presupuestos AWS (mensual/anual/semanal) con alertas al 70/80/90% vía SNS |
| [`scripts/check-tags.py`](../../../scripts/check-tags.py) | Valida tags FinOps obligatorios en el plan de Terraform (usado en CI) |
...
| [`docs/database.md`](../../database.md) | Schema PostgreSQL, migraciones DDL y seeds — cómo se inicializa la base de datos |
| [`docs/caching.md`](../../caching.md) | Estrategia de caché Redis: claves, TTL, degradación elegante y cuándo se invalida |
