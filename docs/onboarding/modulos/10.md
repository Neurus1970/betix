...
| [`docs/caching.md`](../../caching.md) | Estrategia de caché Redis: cómo fluye un MISS y cómo opera en modo degradado cuando Redis no está disponible |
...
El repositorio tiene un catálogo de bugs reales y oportunidades de mejora identificadas en la plataforma, organizadas como reportes de bug listos para convertirse en tickets de Jira y trabajar en el ciclo completo que enseña el curso.

Cada bug tiene descripción del problema, código de referencia, comportamiento esperado y una sección **"Qué demuestra al alumno"** que conecta la mejora con los principios que ya conocés.

→ **[`docs/mejoras/`](../../mejoras/README.md)** — 23 bugs clasificados por área (Microservicios, Testing, Infra, Frontend), prioridad e impacto didáctico.

El flujo sugerido para practicar:

1. Elegir un bug del catálogo que te resulte interesante
2. Crear el ticket en Jira (proyecto BETIX)
3. Crear la rama con el patrón correcto: `fix/BETIX-XX-descripcion`
4. Implementar el fix siguiendo el ciclo completo del Capítulo 9
5. PR → revisión → merge → CI → release
