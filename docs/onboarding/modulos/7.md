...
```mermaid
flowchart LR
    DEV["Developer\ncrea rama"] -->|"git checkout -b"| HOOK["pre-bash.sh\nhook local"]
    HOOK -->|"✅ nombre válido"| PUSH["git push"]
    HOOK -->|"❌ nombre inválido\nexit 2"| BLOCKED["Bloqueado\nen local"]
    PUSH --> PR["Pull Request\na develop"]
    PR --> CI["validate-branch-name.yml\nGitHub Actions"]
    CI -->|"✅ patrón correcto"| MERGE["PR puede mergearse"]
    CI -->|"❌ patrón incorrecto"| FAIL["Check falla\nPR bloqueado"]
...
```mermaid
flowchart RL
    BUG["Bug detectado\nen producción"] -->|"¿qué commit lo introdujo?"| COMMIT["git log / git blame\nsha: abc1234"]
    COMMIT -->|"¿a qué PR pertenece?"| PR["PR #87\nfeature/BETIX-35-..."]
    PR -->|"¿qué ticket resolvía?"| TICKET["BETIX-35\n'Agregar filtro por provincia'"]
    TICKET -->|"¿por qué se decidió así?"| CONTEXT["Descripción del ticket\ncomentarios, criterios de aceptación"]
