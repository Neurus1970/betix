...
graph TD
    subgraph Ramas permanentes
        MAIN["🔒 main\nSolo recibe merges de develop\nCada merge genera un release\nNUNCA se trabaja directo acá"]
        DEV["🔀 develop\nRama de integración activa\nSiempre debe estar en verde\nBase para todas las ramas de trabajo"]
    end
...
graph LR
    subgraph "Tags en ECR"
        SHA["sha-abc1234\nBuild automático de develop/feature\nTrazable al commit exacto"]
        RC["1.3.0-rc.1\nRelease candidate\nPara validar en UAT antes de prod"]
        STABLE["1.3.0\nRelease estable\nSe genera al mergear PR de release"]
        LATEST["latest\nApunta al último release estable\nSiempre sincronizado con el tag más reciente"]
    end
