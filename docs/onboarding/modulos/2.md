# Git como contrato de equipo

### Capítulo 2

← [Volver al temario](../TOC.md)

---

## Objetivos de este capítulo

Al terminar este capítulo vas a poder:
- Entender la estrategia de branching de la plataforma y por qué está diseñada así
- Crear ramas con el nombre correcto y vincularlas a tickets de Jira
- Escribir commits que Release Please entienda para generar versiones automáticas
- Abrir [Pull Requests](../glosario.md#pull-request-pr) que sean revisables y no bloqueen el flujo del equipo
- Aplicar [hotfixes](../glosario.md#hotfix) sobre versiones en producción sin romper el historial

> **Nota:** Las convenciones de este módulo (nombres de ramas, commits, PR flow) son **estándares de la plataforma Tecnoaccion** — no son específicas de Betix. Betix las implementa como proyecto de referencia. Cuando trabajés en otro proyecto del equipo, encontrarás las mismas reglas.

---

## 1. ¿Por qué Git es un contrato, no una herramienta?

Git no es solo "donde guardás código". En la plataforma de Tecnoaccion, Git es la fuente de verdad de **qué cambió, cuándo, por qué y quién lo autorizó**.

Cuando el nombre de una rama incluye `BETIX-42`, Jira mueve el ticket a *In Progress* automáticamente. Cuando el mensaje del commit empieza con `feat:`, [Release Please](../glosario.md#release-please) sabe que debe [bumpar](../glosario.md#bump-de-versión) el minor. Cuando el PR llega a `develop`, el CI verifica que nada se rompió. Todo esto funciona **solo si el equipo respeta las convenciones**.

```mermaid
graph LR
    DEV(["Developer"]) -->|"crea rama<br/>BETIX-42"| JIRA["Jira<br/>move a In Progress"]
    DEV -->|"commit feat:<br/>new endpoint"| RP["Release Please<br/>bumpa MINOR"]
    DEV -->|"abre PR<br/>a develop"| CI["GitHub Actions<br/>corre tests + lint"]
    CI -->|"CI verde"| MERGE["Merge<br/>Jira Done"]
    MERGE -->|"merge a main"| RELEASE["Release<br/>CHANGELOG + tag ECR"]

    style DEV fill:#e8f4f8,stroke:#1976D2
    style JIRA fill:#fff3e0,stroke:#F57F17
    style RP fill:#e8f5e9,stroke:#388E3C
    style CI fill:#fce4ec,stroke:#C62828
    style MERGE fill:#f3e5f5,stroke:#6A1B9A
    style RELEASE fill:#e0f2f1,stroke:#00695C
```

> **Cada convención tiene una razón**. No son reglas arbitrarias — cada una dispara una automatización que le ahorra trabajo al equipo.

---

## 2. La estrategia de branches de Tecnoaccion

La plataforma adopta una versión simplificada de **Git Flow**: dos ramas permanentes y ramas de corta vida por tarea. Betix implementa exactamente esta estrategia — los ejemplos de esta sección son del historial real del proyecto.

```mermaid
gitGraph
   commit id: "init"

   branch develop
   checkout develop
   commit id: "setup proyecto"

   branch feature/BETIX-10-geodata
   checkout feature/BETIX-10-geodata
   commit id: "feat: geodata endpoint"
   commit id: "test: geodata pytest"

   checkout develop
   merge feature/BETIX-10-geodata id: "PR #1 → merge"

   branch feature/BETIX-11-proyecciones
   checkout feature/BETIX-11-proyecciones
   commit id: "feat: SMA rolling"

   branch fix/BETIX-12-timeout
   checkout fix/BETIX-12-timeout
   commit id: "fix: redis timeout"

   checkout develop
   merge fix/BETIX-12-timeout id: "PR #2 → merge"
   merge feature/BETIX-11-proyecciones id: "PR #3 → merge"

   checkout main
   merge develop id: "release 1.1.0" tag: "betix-api-v1.1.0"

   branch hotfix/BETIX-55-fix-sma
   checkout hotfix/BETIX-55-fix-sma
   commit id: "fix: SMA division por cero"

   checkout main
   merge hotfix/BETIX-55-fix-sma id: "PR hotfix → main" tag: "betix-api-v1.1.1"

   checkout develop
   merge hotfix/BETIX-55-fix-sma id: "cherry-pick → develop"
```

### Las dos ramas permanentes

```mermaid
graph TD
    subgraph Ramas permanentes
        MAIN["🔒 main<br/>Solo recibe merges de develop<br/>Cada merge genera un release<br/>NUNCA se trabaja directo acá"]
        DEV["🔀 develop<br/>Rama de integración activa<br/>Siempre debe estar en verde<br/>Base para todas las ramas de trabajo"]
    end

    subgraph Ramas de corta vida
        FEAT["feature/BETIX-XX-...<br/>Nueva funcionalidad"]
        FIX["fix/BETIX-XX-...<br/>Corrección de bug"]
        REFACTOR["refactor/BETIX-XX-...<br/>Reestructuración sin cambio de comportamiento"]
        HOTFIX["hotfix/BETIX-XX-...<br/>Fix urgente en producción"]
    end

    DEV --> FEAT
    DEV --> FIX
    DEV --> REFACTOR
    MAIN --> HOTFIX

    FEAT -->|"PR + review"| DEV
    FIX -->|"PR + review"| DEV
    REFACTOR -->|"PR + review"| DEV
    HOTFIX -->|"PR + review"| MAIN
    HOTFIX -.->|"cherry-pick<br/>si aplica"| DEV

    style MAIN fill:#ffebee,stroke:#b71c1c,color:#b71c1c
    style DEV fill:#e8f5e9,stroke:#1b5e20,color:#1b5e20
    style FEAT fill:#e3f2fd,stroke:#0d47a1
    style FIX fill:#fff8e1,stroke:#e65100
    style REFACTOR fill:#f3e5f5,stroke:#4a148c
    style HOTFIX fill:#fce4ec,stroke:#880E4F
```

### Regla de oro

> `main` solo recibe merges de `develop`. **Nunca** se pushea directamente a `main`.

La única excepción es un fix urgente en producción. En ese caso el flujo es:

1. Crear `hotfix/BETIX-XX-...` **desde `main`** (no desde `develop`)
2. Aplicar el fix mínimo y abrí PR contra `main`
3. Una vez mergeado y taggeado, **[cherry-pick](../glosario.md#cherry-pick)** del fix a `develop` para que no se pierda

> El hotfix sale de `main` porque `develop` puede tener cambios no validados que no querés llevar a producción. Ver el flujo completo con ejemplos en la [sección 6 de este módulo](#6-versionado-con-tags--hotfixes-en-versiones-en-producción).

---

## 3. Anatomía de un nombre de rama

```
feature / BETIX-42 / nueva-visualizacion-sunburst
   │           │                │
   │           │                └── descripción corta en kebab-case
   │           │                    (qué hace, no cómo lo hace)
   │           │
   │           └── ID del ticket en Jira
   │               (dispara la automatización Jira ↔ GitHub)
   │
   └── prefijo que define el tipo de cambio
       (define el tono del commit message y el tipo de bump)
```

| Prefijo | Cuándo usarlo | Bump de versión asociado |
|---------|--------------|--------------------------|
| `feature/` | Nueva funcionalidad | `feat:` → minor bump |
| `fix/` | Corrección de bug | `fix:` → patch bump |
| `refactor/` | Reestructuración sin cambio de comportamiento | `refactor:` → sin bump |
| `hotfix/` | Fix urgente directamente sobre producción | `fix:` → patch bump en main |

> La parte descriptiva de la rama usa [kebab-case](../glosario.md#kebab-case): palabras en minúsculas separadas por guiones (`nueva-visualizacion-sunburst`, no `NuevaVisualizacionSunburst`).

**Ejemplos válidos:**
```bash
feature/BETIX-42-dashboard-sunburst
fix/BETIX-17-redis-timeout-null-pointer
refactor/BETIX-33-extraer-servicio-geodata
hotfix/BETIX-55-fix-division-por-cero-sma
```

**Ejemplos inválidos:**
```bash
mi-rama              # sin prefijo ni Jira ID — no dispara automatizaciones
feature/nueva-cosa   # sin Jira ID — el ticket no se mueve en Jira
BETIX-42             # sin prefijo — ambiguo, CI no puede inferir el tipo
```

### Flujo completo desde cero

```bash
# 1. Siempre partir desde develop actualizado
git checkout develop
git pull origin develop

# 2. Crear la rama con el nombre correcto
git checkout -b feature/BETIX-42-dashboard-sunburst

# 3. Trabajar, commitear (ver siguiente sección)
# ...

# 4. Push y abrir PR
git push origin feature/BETIX-42-dashboard-sunburst
# → En GitHub: abrir PR contra develop
```

---

## 4. Conventional Commits — el mensaje que trabaja por vos

El mensaje de un commit no es solo documentación. En la plataforma, **[Release Please](../glosario.md#release-please) lee los mensajes para decidir qué versión asignar y qué entra en el [CHANGELOG](../glosario.md#changelog)**. Esta herramienta está configurada en todos los proyectos del equipo — Betix incluido.

### El formato

```
<tipo>[alcance opcional]: <descripción en imperativo>

[cuerpo opcional — explicar el "por qué", no el "qué"]

[footer opcional — BREAKING CHANGE o refs a issues]
```

### Los tipos y su impacto

```mermaid
graph LR
    subgraph "Commits que generan versión"
        FEAT["feat: nuevo endpoint /ranking<br/>bump MINOR: 1.2.0 → 1.3.0"]
        FIX["fix: redis timeout en geodata<br/>bump PATCH: 1.2.0 → 1.2.1"]
        BREAK["feat!: cambiar schema respuesta geodata<br/>BREAKING CHANGE: ventas → tickets<br/>bump MAJOR: 1.2.0 → 2.0.0"]
    end

    subgraph "Commits sin bump (aparecen en CHANGELOG)"
        REF["refactor: extraer servicio de caché"]
        DOCS["docs: actualizar README con nuevos endpoints"]
        CHO["chore: actualizar dependencias npm"]
        TEST["test: agregar pytest para edge case SMA"]
    end

    style FEAT fill:#e8f5e9,stroke:#388E3C
    style FIX fill:#fff8e1,stroke:#F57F17
    style BREAK fill:#ffebee,stroke:#C62828
    style REF fill:#e3f2fd,stroke:#1976D2
    style DOCS fill:#f3e5f5,stroke:#6A1B9A
    style CHO fill:#efebe9,stroke:#4E342E
    style TEST fill:#e0f2f1,stroke:#00695C
```

### Ejemplos reales del proyecto

```bash
# Nueva funcionalidad — bumpa MINOR
feat(core): add /ranking endpoint with top 5 provinces by revenue

# Corrección — bumpa PATCH
fix(api): handle null redis response in geodata cache

# Refactor — sin bump, aparece en CHANGELOG
refactor(core): extract SMA calculation to services/sma.py

# Documentación — sin bump
docs: add C4 architecture diagrams to docs/

# Tests — sin bump (pero IMPORTANTE para CI)
test(core): add pytest for SMA edge cases with single data point

# Breaking change — bumpa MAJOR
feat(api)!: rename /datos/geodata response field ventas → tickets_totales

BREAKING CHANGE: el campo 'ventas' en la respuesta de /api/datos/geodata
fue renombrado a 'tickets_totales' para consistencia con el schema de DB.
Actualizar cualquier cliente que dependa de ese campo.
```

> **Tip:** Si no sabés qué tipo usar, preguntale a Claude: _"Este cambio que hice en `core/services/sma.py` es un fix o un refactor? Te paso el diff..."_

> Ver en el glosario: [Conventional Commits](../glosario.md#conventional-commits) · [BREAKING CHANGE](../glosario.md#breaking-change) · [CHANGELOG](../glosario.md#changelog).

### ¿Por qué el mensaje importa tanto?

```mermaid
sequenceDiagram
    participant Dev as Developer
    participant Git as Git
    participant RP as Release Please
    participant GH as GitHub Release

    Dev->>Git: feat(core): add /ranking endpoint
    Dev->>Git: fix(api): redis null pointer
    Dev->>Git: refactor: extraer servicio
    Note over Git: 3 commits en develop

    Git->>RP: merge develop a main
    RP->>RP: feat detectado → bump MINOR
    RP->>RP: fix detectado → bump PATCH
    RP->>GH: Abre PR "chore: release 1.3.0"

    Note over GH: CHANGELOG generado con Features y Bug Fixes

    Dev->>GH: Mergear PR de release
    GH-->>GH: Tag betix-api-v1.3.0 creado en ECR
```

---

## 5. Pull Requests — qué revisar, qué no bloquear

Un Pull Request en Betix no es solo un trámite. Es el momento donde el equipo verifica que el cambio es correcto y seguro antes de integrarlo.

### Checklist del autor (antes de abrir el PR)

```
[ ] La rama sale de develop actualizado (git pull origin develop)
[ ] El nombre de la rama tiene prefijo + BETIX-XX + descripción
[ ] Todos los commits tienen formato Conventional Commits
[ ] make test pasa en local (o al menos make test-core / make test-api según qué tocaste)
[ ] make lint sin errores
[ ] No hay console.log en código JS de producción
[ ] No hay lógica de negocio en src/ (solo en core/)
[ ] Si agregaste un endpoint: está documentado en README.md
```

### Qué revisar como reviewer

```mermaid
graph TD
    PR["📬 PR abierto contra develop"]

    PR --> SCOPE["¿El cambio hace lo que dice el ticket?"]
    PR --> TESTS["¿Hay tests que cubran el comportamiento nuevo?"]
    PR --> ARCH["¿Respeta la arquitectura?<br/>(lógica en core, no en src)"]
    PR --> STYLE["¿Respeta las convenciones de código?<br/>(quotes, indent, logger)"]
    PR --> RISK["¿Tiene riesgo de romper algo existente?"]

    SCOPE -->|"No"| CHANGE["🔴 Solicitar cambios"]
    TESTS -->|"No"| CHANGE
    ARCH -->|"No"| CHANGE
    STYLE -->|"Menor"| COMMENT["💬 Comentario no bloqueante"]
    RISK -->|"Sí"| DISCUSS["💬 Discutir en el PR antes de mergear"]

    SCOPE -->|"Sí"| OK["✅ Aprobar"]
    TESTS -->|"Sí"| OK
    ARCH -->|"Sí"| OK

    style CHANGE fill:#ffebee,stroke:#C62828
    style COMMENT fill:#fff8e1,stroke:#F57F17
    style DISCUSS fill:#e3f2fd,stroke:#1976D2
    style OK fill:#e8f5e9,stroke:#388E3C
```

### Qué NO bloquear

- **Estilo personal** en comentarios o nombres de variables que no violan las convenciones documentadas
- **Optimizaciones prematuras** — si el código es correcto y tiene tests, mergear. Refactorear después con su propio ticket
- **Casos edge hipotéticos** — si no hay un test que lo demuestre como bug, no es bloqueante

> **Regla:** un PR con CI verde + tests que cubren el cambio + al menos una aprobación humana → se mergea. No existe el PR "perfecto".

---

## 6. Versionado con tags — hotfixes en versiones en producción

Este es el caso más delicado del ciclo de vida: tenés **v1.2.0 en producción**, el equipo ya avanzó a `develop` con cambios para v1.3.0, y aparece un bug crítico en prod.

### Los [tags](../glosario.md#tag-git) de versión en [ECR](../glosario.md#ecr-elastic-container-registry)

```mermaid
graph LR
    subgraph "Tags en ECR"
        SHA["sha-abc1234<br/>Build automático de develop/feature<br/>Trazable al commit exacto"]
        RC["1.3.0-rc.1<br/>Release candidate<br/>Para validar en UAT antes de prod"]
        STABLE["1.3.0<br/>Release estable<br/>Se genera al mergear PR de release"]
        LATEST["latest<br/>Apunta al último release estable<br/>Siempre sincronizado con el tag más reciente"]
    end

    STABLE -->|"el más reciente"| LATEST

    style SHA fill:#efebe9,stroke:#4E342E
    style RC fill:#e3f2fd,stroke:#1976D2
    style STABLE fill:#e8f5e9,stroke:#388E3C
    style LATEST fill:#fff8e1,stroke:#F57F17
```

> **Glosario:** [SHA](../glosario.md#sha-commit-hash) · [RC (Release Candidate)](../glosario.md#rc-release-candidate) · [Tag de Git](../glosario.md#tag-git) · [Semver](../glosario.md#semver-semantic-versioning) · [UAT](../glosario.md#uat-user-acceptance-testing) — términos que aparecen en el diagrama de tags.

### Escenario: hotfix sobre versión en producción

Situación: `main` está en `v1.2.0`. El equipo tiene 5 commits en `develop` para v1.3.0. Aparece un bug crítico en v1.2.0.

**¿Qué NO hacer?**  Mergear `develop` a `main` — estarías llevando a producción 5 cambios no validados solo para poder deployar el fix.

**El flujo correcto:**

```mermaid
gitGraph
   commit id: "v1.1.0" tag: "betix-api-v1.1.0"
   commit id: "v1.2.0" tag: "betix-api-v1.2.0"

   branch develop
   checkout develop
   commit id: "feat: ranking endpoint"
   commit id: "refactor: sma service"
   commit id: "feat: nuevo filtro fechas"

   checkout main
   branch hotfix/BETIX-55-fix-division-cero
   checkout hotfix/BETIX-55-fix-division-cero
   commit id: "fix: division por cero en SMA"
   commit id: "test: caso con un solo mes de datos"

   checkout main
   merge hotfix/BETIX-55-fix-division-cero id: "PR hotfix → main" tag: "betix-api-v1.2.1"

   checkout develop
   merge hotfix/BETIX-55-fix-division-cero id: "cherry-pick fix a develop"

   checkout main
   merge develop id: "release v1.3.0" tag: "betix-api-v1.3.0"
```

**Paso a paso:**

```bash
# 1. Partir desde el tag de producción (no desde develop)
git checkout main
git pull origin main
git checkout -b hotfix/BETIX-55-fix-division-cero

# 2. Aplicar el fix mínimo necesario
# ... editar solo lo necesario ...
git add core/services/sma.py core/tests/test_sma.py
git commit -m "fix(core): avoid division by zero in SMA with single data point"

# 3. PR a main (no a develop)
git push origin hotfix/BETIX-55-fix-division-cero
# → Abrir PR contra main en GitHub

# 4. Una vez mergeado y taggeado como v1.2.1,
#    llevar el fix a develop para que no se pierda
git checkout develop
git cherry-pick <hash-del-fix-commit>
git push origin develop
```

### ¿Cuándo hacer rollback en lugar de hotfix?

```mermaid
graph TD
    BUG["🚨 Bug crítico en producción"]

    BUG --> ASSESS["¿El fix es de 1-2 líneas y está claro?"]

    ASSESS -->|"Sí"| HOTFIX_FLOW["✅ Hotfix: rama hotfix/ desde main<br/>corregir, PR, tag v1.2.1"]
    ASSESS -->|"No / complejo"| ROLLBACK["⏪ Rollback: redesployar<br/>betix-api:1.1.0 (tag anterior)<br/>luego investigar con calma"]

    HOTFIX_FLOW --> CHERRY["Cherry-pick del fix a develop<br/>para no perderlo"]
    ROLLBACK --> ROOT["Investigar causa raíz<br/>con el sistema estable"]

    style HOTFIX_FLOW fill:#e8f5e9,stroke:#388E3C
    style ROLLBACK fill:#fff8e1,stroke:#F57F17
    style CHERRY fill:#e3f2fd,stroke:#1976D2
    style ROOT fill:#f3e5f5,stroke:#6A1B9A
```

> **Regla:** si no podés corregir y testear el fix en menos de una hora, hacé [rollback](../glosario.md#rollback) primero. Los tags de ECR hacen que eso sea trivial: `kubectl set image deployment/betix-api api=betix-api:1.1.0`.

---

## 7. Ejercicio — Crear una rama y escribir el commit con Claude

En este ejercicio vas a simular el inicio de una tarea real en Betix.

### Contexto del ticket ficticio

> **BETIX-99:** Como analista, quiero que el endpoint `/healthz` incluya la versión del servicio en la respuesta, para poder verificar qué imagen está corriendo sin acceder a los logs.

### Paso 1: Crear la rama correctamente

```bash
git checkout develop
git pull origin develop
git checkout -b feature/BETIX-99-healthz-version-info
```

### Paso 2: Hacer el cambio mínimo

El endpoint `/healthz` está en `core/main.py`. Abrí el archivo y localizá la función que responde ese endpoint.

```bash
# En lugar de buscar manualmente, preguntale a Claude:
```

Prompt para Claude:

```
Estoy implementando el ticket BETIX-99: agregar la versión del servicio
a la respuesta del endpoint /healthz en core/main.py.

1. Encontrá la función que maneja ese endpoint
2. Mostrarne cómo leer el contenido del archivo core/VERSION
3. Sugerí la mínima modificación para incluir {"status": "ok", "version": "1.2.0"}
   sin romper los tests existentes
```

### Paso 3: Escribir el commit message con ayuda de Claude

Una vez que hiciste el cambio:

```bash
git diff core/main.py
```

Prompt para Claude:

```
Hice el siguiente cambio en core/main.py (te paso el diff).
Necesito escribir un Conventional Commit message para este cambio
en el proyecto Betix. Recordá que:
- feat: bumpa MINOR
- fix: bumpa PATCH
- Los mensajes van en inglés
- Formato: tipo(scope): descripción en imperativo

¿Cuál sería el mensaje correcto y por qué?
```

### Paso 4: Commitear y pushear

```bash
git add core/main.py
git commit -m "feat(core): include service version in /healthz response"

git push origin feature/BETIX-99-healthz-version-info
# → Abrir PR contra develop en GitHub
```

### Paso 5: Reflexión

Respondé estas preguntas antes de continuar:

1. Si este cambio llega a `main`, ¿qué tag de versión se va a generar para `betix-core`? ¿Por qué?
2. ¿Debería correr `ci-core.yml` o `ci-api.yml` para este cambio?
3. Si mañana aparece un bug en prod en `betix-core:1.2.0` mientras `develop` ya tiene 3 commits nuevos, ¿cómo lo corregís?

> **Verificá tus respuestas con Claude:** _"Revisá si mi razonamiento sobre el versionado y el hotfix es correcto..."_

---

## Resumen

```mermaid
mindmap
  root((Módulo 2))
    Branching strategy
      develop como base
      feature/ fix/ refactor/ hotfix/
      BETIX-XX en el nombre
      Jira se mueve automáticamente
    Conventional Commits
      feat → MINOR bump
      fix → PATCH bump
      BREAKING CHANGE → MAJOR
      Release Please los lee
    Pull Requests
      CI verde obligatorio
      Una aprobación humana
      No bloquear por estilo
    Tags y hotfixes
      sha- para builds CI
      v1.2.0 para releases
      latest para producción
      hotfix/ desde main
      cherry-pick a develop
```

---

## Recursos del repositorio

| Recurso | Descripción |
|---------|-------------|
| [`CLAUDE.md — Git/Branching`](../../../CLAUDE.md#git--branching) | Reglas de branching del proyecto |
| [`docs/monorepo-guide.md`](../../monorepo-guide.md) | Versionado independiente por servicio y convención de tags ECR |
| [`docs/SDLC.md`](../../SDLC.md) | Ciclo de vida completo del desarrollo |
| [`docs/principios-fundamentales.md`](../../principios-fundamentales.md) | Los 5 principios que explican por qué cada convención de Git existe y qué automatización dispara |
| [`.github/workflows/`](../../../.github/workflows/) | Los dos workflows de CI y sus path filters |
| [`CHANGELOG.md`](../../../CHANGELOG.md) | Historial generado automáticamente por Release Please |
| [**Glosario de términos técnicos**](../glosario.md) | Definiciones de bump, cherry-pick, semver, RC, SHA y más |

---

← [Módulo 1](1.md) | [Módulo 3](3.md) →
