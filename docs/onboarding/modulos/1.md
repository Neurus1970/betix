...
graph
    ...
    C -->|"Leer"| T1["Ver tickets del sprint<br/>¿Qué hay en To Do?"]
    C -->|"Leer"| T2["Ver detalle de un ticket<br/>¿Qué pide BETIX-42?"]
    C -->|"Crear"| T3["Crear un ticket<br/>Con título, descripción y criterios"]
    C -->|"Transicionar"| T4["Mover estado<br/>To Do → In Progress"]
    C -->|"Buscar"| T5["Buscar por JQL<br/>Bugs sin asignar del sprint"]
...
graph LR
    ...
    subgraph Betix
        FE["🌐 Frontend<br/>nginx :8080<br/>Sirve HTML/CSS/JS + D3.js"]
        API["⚙️ Betix API<br/>Node.js :3000<br/>Proxy HTTP + caché Redis"]
        CORE["🧠 Betix Core<br/>Python Flask :5000<br/>Toda la lógica de negocio"]
        REDIS[("🔴 Redis :6379<br/>Caché de respuestas")]
        DB[("🐘 PostgreSQL :5432<br/>Datos de tickets")]
    end
...
```mermaid
graph TD
    L1["Nivel 1 — Contexto<br/>¿Qué es Betix y quién lo usa?<br/>Audiencia: negocio"]
    L2["Nivel 2 — Contenedores<br/>¿Qué servicios lo componen?<br/>Audiencia: arquitectos"]
    L3["Nivel 3 — Componentes<br/>¿Qué hay dentro de cada servicio?<br/>Audiencia: desarrolladores"]
    L4["Nivel 4 — Código<br/>Clases, funciones, detalles<br/>Audiencia: IDE / revisión"]
...
| [`docs/caching.md`](../../caching.md) | Estrategia de caché Redis entre Node.js proxy y Python core |
| [`docs/database.md`](../../database.md) | Schema PostgreSQL, migraciones y seeds |
