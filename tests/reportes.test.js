...
describe('GET /api/reportes/provincias', () => {
  ...
  it('Buenos Aires debe tener el mayor ingreso', async () => {
    const res = await request(app).get('/api/reportes/provincias?fechaInicio=2026-01-01&fechaFin=2026-12-31');
    expect(res.body.data[0].provincia).toBe('Buenos Aires');
  });

  // --- Happy path: filtros por fecha ---
  ...
  // --- Happy path: filtro por tipoJuego (case-insensitive) ---
  it('debe filtrar por tipoJuego de forma case-insensitive', async () => {
    const resLower = await request(app).get('/api/reportes/provincias?fechaInicio=2026-01-01&fechaFin=2026-12-31&tipoJuego=quiniela');
    const resUpper = await request(app).get('/api/reportes/provincias?fechaInicio=2026-01-01&fechaFin=2026-12-31&tipoJuego=QUINIELA');
  ...
  it('debe usar defaults cuando no se pasan fechas', async () => {
    const res = await request(app).get('/api/reportes/provincias');
    expect(res.statusCode).toBe(200);
    expect(res.body.status).toBe('success');
  });
  ...
  it('debe retornar array vacío con mensaje cuando no hay datos', async () => {
  ...
  it('debe retornar 400 para una fecha imposible', async () => {
